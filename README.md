...
- **撤销**：内存栈记录每次应用，一键恢复
- **目录筛选**：包含/排除关键词（支持正则）、仅文件/仅文件夹、是否递归子目录
- 右键预览行可"打开所在文件夹"定位文件
...
1. 输入或浏览选择目标文件夹，点击「加载」；可勾选子文件夹/仅文件/仅文件夹，并用"包含/排除"过滤列表
2. 在左侧选择规则类型 → 点击「添加」→ 在下方"规则参数"区填写 → 点「保存修改」
3. 右侧预览即时更新：绿色=就绪，红色=冲突/错误，灰色=无变化
4. 确认无误后点击「应用重命名」；如需恢复，点击「撤销上次」
...
```bash
python -m unittest tests.test_engine   # 20 个用例：规则转换 / 编号索引 / 作用范围 / 冲突 / 互换重命名 / 撤销
```
